...
| video generation | `services["video-generation"].apiBase` | `services["video-generation"].model` | `xai` (`credential/xai/api_key`, `XAI_API_KEY`); `openai-compatible` reuses the `openai` key over the same xAI-shaped async API, keyless-local allowed |
